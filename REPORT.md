...
### 2.3 텍스처 & UV 매핑 — 등불(등롱) 에셋
- 플레이어가 든 등불은 **UV 매핑된 텍스처를 가진 glTF 에셋**(`public/models/lantern.glb`)이다. 나무 프레임·종이 갓·금속 손잡이가 하나의 diffuse 텍스처 맵에 UV로 펼쳐져 있어 강의의 **텍스처 매핑/UV** 단계에 직접 대응한다.
- **emissive 맵 트릭:** 같은 diffuse 맵을 `emissiveMap`으로 재사용해, 텍셀 밝기로 발광을 변조한다 → **밝은 종이 갓만 빛나고 어두운 나무 틀은 발광하지 않는다**(단일 메시·단일 머티리얼에서 부분 발광 구현). §3.3 참조.
- 벽·바닥은 의도적으로 단색 머티리얼(단청 적색/흙색)을 유지했다: DDGI의 **색 번짐(color bleeding)** 을 텍스처 노이즈 없이 또렷이 보여 주기 위함.

![등불 텍스처 에셋 — UV 텍스처 + emissive 종이 갓](captures/lantern_texture.png)
